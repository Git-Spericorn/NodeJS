...
app.post('/updateMember/:id', async (req, res, next) => {
    try {
        ...
        return res.json({
            success: true,
            message: `updated successfully a member with id = ${id}`,
        });
    } catch (e) {
        if (e.name === 'ValidationError') {
            return res.status(422)
...
app.post('/deleteMember', async (req, res, next) => {
    try {
        await validator(req.query);
        ...
        return res.json({
            success: true,
            message: `deleted successfully a member with id = ${id}`,
        });
    } catch (e) {
        if (e.name === 'ValidationError') {
            return res.status(422)
